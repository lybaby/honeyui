...
<template>
  <div>
    ...
    <el-select v-model="value" placeholder="请选择" :border="true">
      <el-option
        v-for="item in options"
        :key="item.value"
        :label="item.label"
...
监听下拉事件
:::demo 列表下拉会触发`on-dropdown`事件
```html
<template>
  <div>
    <el-select v-model="value" placeholder="请选择" :border="true" @on-dropdown="onDropdown">
      <el-option
        v-for="item in options"
        :key="item.value"
        :label="item.label"
...
<script>
  export default {
    ...
    methods: {
      onDropdown() {
        console.log('you drop down the list');
      }
    }
  }
</script>
