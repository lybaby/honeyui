...
在回显的时候，组件只对 **首次赋值** 的变量进行路径自动搜寻，无法响应因 **外部变化** 引起的 `value` 变更，且不支持异步赋值。可考虑通过 `v-if` 控制组件在数据拉取后再进行组件渲染，或通过赋值 `v-key` ，并在数据拉取完成后刷新的方式让组件重新渲染。
...
```html
<div class="block">
  <span class="demonstration">lazy动态加载</span>
  <el-cascader :props="props"></el-cascader>
</div>
<div class="block">
  <span class="demonstration">变量回显</span>
  <el-cascader v-model="defaultValue" :props="props1"></el-cascader>
</div>
          ...
          lazyLoad(node, resolve) {
            ...
            setTimeout(() => {
              const nodes = Array.from({ length: level + 1 }).map((item, index) => ({
                value: level * 10 + index,
                label: `选项${level * 10 + index}`,
                leaf: level >= 2
              }));
              // 通过调用resolve将子节点数据返回，通知组件数据加载完成
              resolve(nodes);
